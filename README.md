...
### 知识图谱多跳推理
- 13,457 实体 / 45万+ 共现边（jieba.posseg 抽取 + 共现矩阵 + IDF 过滤）
...
**当前结果** (同 k 口径, 扩语料后干净库实测): GraphRAG 比纯向量 RAG 总体 Recall@5 **+2.5%**（76.3%→78.8%），实体关联类 **+5.9%**，对比分析类 **+5.5%**（扩语料后由负转正）；负样本诚实拒答率 **80%**，幻觉陷阱 100% 拦截。
...
> ⚠️ 评测数据依赖全量语料（287 篇文章）。clone 后先运行 `curl -X POST http://localhost:8010/tools/force_update -d '{}'` 拉取数据，再跑评测复现。
...
| 全量文章 (~287 篇) | `pipeline/data/articles/` | ❌ gitignore，`force_update` 生成 |
