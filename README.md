...
| LLM-as-Judge | Answerability (0-1) | 50 条查询, 三臂对比 |
...
> **诚实说明**: 小语料下三路融合对关键词召回提升有限（BGE-Reranker 主导最终排序），GraphRAG 的价值在于多跳推理类查询（+8.3%）、KG 多跳推理链（Agent 工具）与社区全局检索。
> **语义口径 (LLM-as-Judge, 修复后评测器, 三臂 50 条)**: Agentic 循环集合级可答率 **26.0%** > 纯向量 22.0% > GraphRAG 14.0%——改写+反思循环带来 +4pp 语义增益，78% 查询触发二轮检索。绝对分数受语料覆盖限制（363 篇答不全 50 条中的部分问题），对比值才是结论。GraphRAG 语义口径落后与其关键词优势互补（图路贡献覆盖率而非单文档答案质量），结果文件本地保留。早期关键词数字曾被"重复入库污染基线"的假象干扰（MD5 GBK 编码 + 子串匹配串档），已修复并重测——详见 commit 历史与 CLAUDE.md。
