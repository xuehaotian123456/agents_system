...
**规模**: 15150 实体 | 50万+ 共现边 | 7,206 chunks
...
### 5.0 LangGraph 质量门控 (LLM 语义判定 + 条件路由)

```
CRAWL → RULE_FILTER → LLM_GATE ──条件边──┬→ INGEST (入库, 原可信度)
                                          ├→ DEMOTE (可信度×0.4 + quality_demoted 标记)
                                          └→ SKIP   (丢弃, 判定理由留痕)
```

- 规则层 (免费): 广告词/长度/纯外链 → 抓格式垃圾
- LLM 层 (批量 5 篇/次): 抓语义垃圾 (八卦/软文/无关内容)
- 三路而非二元: judge 误判时 demote 通道"宁可降权不误杀"
- 成本控制: 内容 hash 判定缓存 (跨轮次复用) + 官方文档 (credibility≥0.9) 免判定快通道
- 故障开放: LLM 不可用 → 全部默认入库, 不阻塞爬取
- 判定留痕: 每篇文档 verdict + reason 写入日志, 可审计
...
| 负样本测试 | 诚实拒答率 | 30 条 (5 类陷阱) |

### 当前结果

**关键词匹配 (50 条, 同 k 口径, 干净库实测)**:
...
> 诚实结论: 4.2k chunks 规模下三路融合对关键词召回提升有限（Reranker 主导）；GraphRAG 价值在实体关联查询、KG 多跳推理链工具与 LLM-as-Judge 语义评估。早期更高数字是重复入库污染的假象（MD5 缓存 GBK/UTF-8 编码 bug + 子串匹配串档），已修复重测。
...
| 组件 | 代码量 | 文件数 |
|:---|:---|:---|
| Interaction 层 | ~10,000 行 | 45 模块 |
| Pipeline 层 | ~10,000 行 | 40+ 模块 |
| 评测体系 | ~1,500 行 | 7 脚本 |
| **总计** | **~23,000 行** | **100 个 .py 文件** |
