...
### 当前结果 (50 条查询, 5089 chunks, 13457 KG 实体, 扩语料后干净库实测)
...
**诚实结论**: 在 5.1k chunks 规模上三路融合 Recall@5 +2.5%（扩语料前 4.2k 时为 +1.3%，对比分析类从 -4% 转正为 +5.5%——语料规模越大图路增量信息越多，验证了权重差假说），优势集中在实体关联（+5.9%）与时效查询（+7.3%）。这符合预期——小语料下向量检索已能命中关键词，且 BGE-Reranker 主导最终排序；GraphRAG 的真实价值在语义相关性（LLM-as-Judge 评估）和 KG 多跳推理链（作为 Agent 工具提供推理路径），而非关键词召回。曾尝试分数级融合调 λ，实测提升 <0.5% 且有测试集过拟合风险，已撤回。
...
| 文章数 | 287 篇 (内容去重后) |
| Chunks | 5,089 |
| KG 实体 | 13,457 (停用词清洗 + 扩语料后) |
| ChromaDB | 5,089 embeddings (1:1) |
| URL 去重 | 251 条 |
...
- Pipeline: ~3500 行 Python，4 节点 + 14 工具，7 仓库爬虫，13,457 KG 实体
