...
> 多次重建重跑基线在 56-65% 间波动（HNSW 近似索引 + embedding 服务漂移），GraphRAG 提升稳定为正。

| 指标 | 纯向量 RAG | GraphRAG | 提升 |
|:---|:---|:---|:---|
| Recall@5 (最新一次) | 65% | 78% | **+13%** |
| Recall@5 (波动区间) | 56-65% | 76-78% | **+13%~+19%** |
| 实体关联类 | 67% | 88% | +22% |
| 报错溯源类 | 94% | 94% | 持平 |
| 多跳推理类 | 62% | 75% | +12% |
...
- **单次跑的数字有 HNSW 近似检索噪声**，跨重建波动 ±5-9 个百分点；报告用区间 + 最近一次实测
